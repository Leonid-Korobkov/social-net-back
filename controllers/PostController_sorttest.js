...
const PostController = {
  ...
  async getAllPosts (req, res) {
    ...
    const sort = req.query.sort || 'newest' // newest, oldest, likes, views, viewed_newest, viewed_oldest
    ...
    try {
      ...
      // Определяем сортировку для Prisma
      let prismaOrderBy = [];

      if (feedType !== 'viewed') {
        // Сортировка для 'new', 'following', 'for-you'
        if (sort === 'likes') {
          prismaOrderBy = [{ likeCount: sortOrder }];
        } else if (sort === 'views') {
          prismaOrderBy = [{ viewCount: sortOrder }];
        } else if (sort === 'oldest') {
          prismaOrderBy = [{ createdAt: 'asc' }];
        } else { // newest или любой другой дефолт
          prismaOrderBy = [{ createdAt: 'desc' }];
        }
      } else {
        // Для 'viewed' сортировка по viewed_newest/viewed_oldest делается в JS.
        // Для likes/views/createdAt в 'viewed' (если они вдруг будут запрошены фронтом) используем orderBy.
        if (sort === 'likes') {
          prismaOrderBy = [{ likeCount: sortOrder }];
        } else if (sort === 'views') {
          prismaOrderBy = [{ viewCount: sortOrder }];
          ...
          prismaOrderBy = [{ createdAt: 'asc' }];
           ...
           // Для viewed_newest/oldest и других неизвестных сортировок в Prisma не сортируем
           prismaOrderBy = [];
        }
      }
      ...
      // Если сортировка по дате просмотра — сортируем вручную
      if (feedType === 'viewed' && (sort === 'viewed_newest' || sort === 'viewed_oldest')) {
        posts.sort((a, b) => {
          const aView = a.PostView[0]?.createdAt ? new Date(a.PostView[0].createdAt).getTime() : 0;
          const bView = b.PostView[0]?.createdAt ? new Date(b.PostView[0].createdAt).getTime() : 0;
          return sort === 'viewed_newest' ? bView - aView : aView - bView;
        });
      }
